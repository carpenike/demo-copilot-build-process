...
src/
  ...
  uv.lock         # Locked dependency versions (generated by uv lock)
  Makefile
```

**Dependency locking:** After creating `pyproject.toml`, run `uv lock` from the
`src/` directory to generate `uv.lock`. This pins exact dependency versions and
prevents incompatible version combinations (e.g., `azure-monitor-opentelemetry`
vs `opentelemetry-sdk`). The Dockerfile should use `uv sync` (not `uv pip install`)
to install from the lockfile. Commit `uv.lock` to the repo.
...
10. **Mypy passes** — run `mypy app/` from `projects/<project>/src/` and verify
    exit code 0. If `mypy` is not installed, install via `uv pip install mypy`.
    Fix all type errors before committing — do not add blanket `type: ignore`
    comments without a specific error code.
11. **Existing tests still pass** — if `projects/<project>/tests/` exists from a
